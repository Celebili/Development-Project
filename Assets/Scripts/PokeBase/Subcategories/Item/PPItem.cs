    ...
    public string AddPP(Pokemon pokemon, int n)
    {
        ...
        if(restoreMax)
        {
            ...
            else
            {
                ...
                return $"{pokemon.NickName}'s {pokemon.Skill[n].Base.SkillName} PP up!";
            }
        }
        else
        {
            if(restoreAll)
            {
                ...
                return $"{pokemon.NickName}'s all skill up {restoreValue.ToString()}!";
            }
            else
            {
                pokemon.Skill[n].PPValueChange(restoreValue);
                return $"{pokemon.NickName}'s {pokemon.Skill[n].Base.SkillName} PP up {restoreValue.ToString()}!";
            }
        }
    }
}
