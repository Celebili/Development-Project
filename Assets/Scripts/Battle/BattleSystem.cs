	...
	private async void SetupBattle()
    {
        ...
        if(!isTrainerBattle)
        {
			...
			enemyUnit.SetData(wildPokemon, false);
				...
				combatText.Append("A wild "); combatText.Append(enemyUnit.Pokemon.NickName);
			    combatText.Append(" approach!"); await ShowCombatText();
        ...
        else
        {
			...
			await enemySprites.PlayNormalAnimator();
                ...
                combatText.Append(trainer.TrainerName); combatText.Append("Start battle!");
			...
			enemyUnit.SetData(enemyPokemon, false);
                ...
                combatText.Append(trainer.TrainerName); combatText.Append(" throw out ");
            ...
            playerUnit.SetData(playerPokemon, false);
                ...
                combatText.Append("Go "); combatText.Append(playerPokemon.NickName);
        ...
        if (isDynamax)//检查是否极巨化过
		{
			...
			if (dynamaxTurnNum > 2)
			{
				...
				isDynamax = false;
                    ...
                    combatText.Append(playerUnit.Pokemon.NickName);combatText.Append("Back to the original form!");
                    await ShowCombatText();
			}
		}

		await dialogBox.TypeDialog("What to do?");
...
#region 释放技能
    ...
    private async UniTask RunMove(BattleUnit sourceUnit, BattleUnit targetUnit, Skill skill)
	{
		...
		if (sourceUnit == playerUnit)//如果是玩家, 就更新技能栏
		{
		    ...
		    //释放技能文本后不用延时1s//但没写
		    combatText.Append(sourcePokemon.NickName); combatText.Append(" use ");
		...
		{
			//target闪避一下//动画没写
			combatText.Append(sourcePokemon.NickName); combatText.Append(" miss target!");
            await ShowCombatText();
		}
	}
...
#region 战斗结束判断debug
    ...
    private async UniTask HandlePokemonFainted(BattleUnit faintedUnit)
	{
		...
		Pokemon fainted = faintedUnit.Pokemon;
		    ...
		    combatText.Append(fainted.NickName);combatText.Append(" falling down");
		    await ShowCombatText();
			...
			if(playerPokemon.Level < 100)//经验计算
			{
            	...
            	float trainerBonus = isTrainerBattle? 1.5f : 1f;//经验加成
          		...
          		playerPokemon.Exp += expGain;//获得经验
				    ...
				    combatText.Append(playerPokemon.NickName); combatText.Append(" gain ");
				    combatText.Append(expGain); combatText.Append(" exp!");
		   	        await ShowCombatText();
				...
				while (playerPokemon.CheckForLevelUp())//升级
				{
					playerUnit.Hud.SetLevel();
					    ...
					    combatText.Append(playerPokemon.NickName); combatText.Append(" up to ");
			    	    combatText.Append(playerPokemon.Level); combatText.Append(" level!");
		        	    await ShowCombatText();
					...
					if (newSkill != null)
					{
						if(playerPokemon.Skill.Count < 4)
						{
							playerPokemon.LearnSkill(newSkill.Base);
							    ...
							    combatText.Append(playerPokemon.NickName); combatText.Append(" learned ");
			      	 	        combatText.Append(newSkill.Base.SkillName); combatText.Append("!");
                   	            await ShowCombatText();
						}
						else
						{
							    combatText.Append(playerPokemon.NickName); combatText.Append(" need to forget a skill!");
							    await ShowCombatText();
		...
		else
		{
		    ...
		    {
				await dialogBox.TypeDialog("Won!");
		    	BattleOver(true);
		    	playerUnit.ResetSR();
		    }
		    else
		    {
		    	...
		    	{
					await dialogBox.TypeDialog("Won!");
					    combatText.Append(trainer.TrainerName); combatText.Append(" I still need learn more...");
					    await ShowCombatText();
	...
	{
		await dialogBox.TypeDialog("defeat...");
		enemyUnit.ResetSR();
		BattleOver(true);
	}
#endregion
#region 替换宝可梦debug
	...
	private async UniTask SwitchPokemon()
	{
		...
		if(playerUnit.Pokemon.Dynamax)//清除极巨化
		{
		    ...
		    combatText.Append("come back "); combatText.Append(playerUnit.Pokemon.NickName);
			combatText.Append("!"); await ShowCombatText();
		...
		playerUnit.SetData(selectedPokemon, isMega && megaNum == selectPokemon);
		    ...
		    combatText.Append("go "); combatText.Append(selectedPokemon.NickName);
			combatText.Append("!"); await ShowCombatText();
...
#region 捕捉宝可梦
    ...
    private async UniTask ThrowPokeBall()
	{
		...
		{
			await dialogBox.TypeDialog("You can not catch other trainer's pokemon...");
			state = BState.RunningTurn;
			return;
		}

		    combatText.Append(player.PlayerName); combatText.Append(" throw out ");
		...
		if(shakeCounts == 4)
		{
			ball.CatchPokemon();
			    ...
			    combatText.Append("Catch "); combatText.Append(enemyPokemon.NickName);
			    combatText.Append("!"); await ShowCombatText();
            ...
            playerTeam.AddPokemon(enemyPokemon);
			    ...
			    {
			    	combatText.Append("put in team!");
			    }
			    else
			    {
			    	combatText.Append("put in box!");
			    }
			    await ShowCombatText();
		...
		else
		{
			...
			if(shakeCounts < 2)
			{
				combatText.Append(enemyPokemon.NickName);combatText.Append(" wrench the ball!");
				await ShowCombatText();
			}
			else
			{
				await dialogBox.TypeDialog("it's close!!!!!");
			}
...
#region 逃跑debug
    ...
    private async UniTask TryToEscape()
	{
		...
		{
			await dialogBox.TypeDialog("Can't back down!");
			state = BState.RunningTurn;
			return;
		}
		...
		{
			await dialogBox.TypeDialog("escape success!");
			BattleOver(true);
		...
		else
		{
			...
			{
				await dialogBox.TypeDialog("Failure to escape!");
				state = BState.RunningTurn;
				if (playerUnit.Pokemon.Status != null)
				{
					await RunAfterTurn(playerUnit);
